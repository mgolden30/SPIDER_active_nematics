function [c_ave, residual_ave, residual_sequence_ave, singular_ave, c_std, residual_std, residual_sequence_std, singular_std] = svd_regression_statistics2( A, B, threshold, subsamples, size_samples )
  %{
  ...
  for sample=1:num_samples
      ...
      small_A = A(y,:);      
      small_A = [small_A; B];
      [c, residual, residual_sequence, singular] = svd_regression(small_A, threshold);
      
      %if( c(1) == 1 )
        cs = [cs c];
        residuals = [residuals residual];
        residual_sequences = [ residual_sequences residual_sequence'];
        singular_vals = [singular_vals singular];
      %end
  end
